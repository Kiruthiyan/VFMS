    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        // ⚠️ TESTING/DEMO MODE: all requests permitted without authentication.
                        // The JWT filter and auth beans remain wired so login works and tokens are
                        // honoured when present. TODO: restore `.anyRequest().authenticated()` (keeping
                        // `/api/auth/**` open) once the frontend injects JWT tokens on requests.
