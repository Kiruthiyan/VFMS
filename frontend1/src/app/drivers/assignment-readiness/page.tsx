...
export default function DriverReadinessPage() {
	...
	const fetchDrivers = async () => {
		setLoading(true);
		try {
			const readinessData = await apiFetch<DriverReadinessCache[]>('/api/drivers/readiness');
			setDrivers(readinessData);

			void apiFetch<PageResponse<Driver>>('/api/drivers?page=0&size=500')
				.then((driversPage) => {
	...
	const ready = drivers.filter((d) => d.ready ?? d.licenseValid);
	...
	const filtered = drivers.filter((d) => {
		...
		return getDisplayDriverId(d.driverId).toLowerCase().includes(query) || d.driverId.toLowerCase().includes(query);
	});
				...
				<CardContent className="p-0">
					{loading ? (
						...
						<Table>
							...
							<TableBody>
								{filtered.map((d) => {
									const isReady = d.ready ?? d.licenseValid;
									return (
