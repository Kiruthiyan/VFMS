...
export function DriverForm({ onSuccess, driver }: { onSuccess: () => void; driver?: any }) {
  ...
  const requiredFields: Array<keyof DriverFormData> = ['firstName', 'lastName', 'nic', 'phone', 'licenseNumber', 'licenseExpiryDate'];
  ...
  const normalizePayload = (data: DriverFormData) => {
    ...
    return isEditing
      ? {
          employeeId: data.employeeId?.trim() || driver.employeeId,
          ...payload,
        }
      : payload;
  };
  ...
  const onSubmit = async (data: DriverFormData) => {
    ...
    try {
      ...
      console.warn('Driver submit failed:', message, e?.response?.data || e);
  ...
  const fields: DriverFieldConfig[] = [
    ...(isEditing
      ? [{ label: 'Employee ID', name: 'employeeId' as const, type: 'text', placeholder: 'Auto-generated employee ID', helperText: 'Generated by the system' }]
      : []),
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 rounded-md bg-white p-4">
      <p className="text-xs font-semibold text-black">Fields marked with * are required. Employee ID is generated automatically. Drivers are added via Admin User Management.</p>
