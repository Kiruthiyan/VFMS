...
import { AlertCircle, FileText, Flag, LayoutDashboard, Plus } from "lucide-react";
import Link from "next/link";
import { usePathname } from "next/navigation";

import { cn } from "@/lib/utils";

const FUEL_MANAGEMENT_ITEMS = [
  { href: "/admin/fuel", label: "Overview", icon: LayoutDashboard },
  { href: "/admin/fuel/logs", label: "Fuel Logs", icon: FileText },
  { href: "/admin/fuel/create", label: "New Entry", icon: Plus },
  { href: "/admin/fuel/alerts", label: "Fuel Alerts", icon: AlertCircle },
  { href: "/admin/fuel/alerts/flagged", label: "Flagged Records", icon: Flag },
];

export function FuelManagementNav() {
  ...
  return (
    ...
    >
      <div className="grid gap-1 p-2 sm:grid-cols-2 xl:grid-cols-5">
        {FUEL_MANAGEMENT_ITEMS.map((item) => {
