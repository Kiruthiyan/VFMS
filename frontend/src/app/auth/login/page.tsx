...
export default function LoginPage() {
  return (
    <main className="auth-page relative min-h-screen overflow-hidden bg-slate-50 px-4 py-10 sm:px-6 lg:px-8">
      <div className="pointer-events-none absolute inset-0 -z-10">
        <div className="auth-glow-left absolute -left-20 top-0 h-72 w-72 rounded-full bg-amber-200/45 blur-3xl" />
        <div className="auth-glow-right absolute -right-24 bottom-0 h-80 w-80 rounded-full bg-slate-300/35 blur-3xl" />
      </div>

      <div className="mx-auto flex min-h-[85vh] w-full max-w-5xl items-center justify-center gap-10 xl:gap-12">
        <section className="auth-card-enter w-full max-w-md rounded-3xl border border-slate-200/80 bg-white/90 p-8 shadow-xl shadow-slate-200/60 backdrop-blur sm:p-10">
          ...
          <footer className="mt-7 border-t border-slate-100 pt-5 text-center">
            ...
            <Link
              href="/"
              className="mt-3 inline-flex text-xs font-semibold text-slate-700 underline-offset-4 hover:text-slate-900 hover:underline"
            >
              Back to Home
            </Link>
          </footer>
        ...
        <aside className="auth-side-enter hidden max-w-sm xl:block">
