...
export default function EditVendorPage() {
  ...
  useEffect(() => {
    vendorApi.getById(Number(id)).then((res) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await vendorApi.update(Number(id), form);
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="p-8 max-w-2xl mx-auto animate-in fade-in duration-500">
        <Button variant="ghost" onClick={() => router.push(`/dashboard/vendors/${id}`)} className="mb-4 text-slate-600 hover:text-slate-900">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex gap-3 pt-3 border-t border-slate-200">
                <Button type="submit" className="bg-blue-950 hover:bg-blue-900 text-white shadow-lg shadow-blue-200" disabled={submitting}>
                  <Save className="mr-2 h-4 w-4" /> {submitting ? "Saving..." : "Update Vendor"}
                </Button>
                <Button type="button" variant="outline" onClick={() => router.push(`/dashboard/vendors/${id}`)}>Cancel</Button>
              </div>
            </form>
          </CardContent>
        </Card>
